...
/*
       super는 부모에게 접근할수 있는 키워드
       자식 클래스에서 "부모님 기능 또는 속성" 접근할 때 쓰는 것

       super 키워드의 두가지 사용 방식
       1. super() : 부모 클래스의 생성자를 호출하는 키워드 /기본 생성자라면 (), 아무런 값이 없다.
            매개변수 생성자라면 () 매개변수 값이 있을 것임.
       2. super.메소드() 또는 super.필드명(멤버변수) : 부모 클래스의 메소드 또는 멤버변수에 접근할수 있음
...
public class Tiger extends Animal{
                                                    ...
                                                    //extends!
                                                //  클래스명 뒤에 extends라고 쓰고 클래스 명(Animal)
                                                //  Animal은 부모(슈퍼)클래스, Tiger은 자식(서브)클래스
                                                //  Tiger는 Animal에게 상속받아 Animal이 갖고있는 속성과 메소드를 그대로 받았음
                                                //  이때 부모클래스에는 없는 속성을 따로 추가할수 있음
...
//    오버라이드. 마찬가지로 Alt+Insert ->메소드 재정의.
//    상속 관계에서 자식 클래스가 부모 클래스의 메소드를 재정의
//    부모에게 물려받은 메소드를 자식이 '다르게' 실행하도록 바꾸는 것
//    부모 클래스의 메소드를 자식클래스에서 동일한 이름, 매개변수, 리턴 타입으로 재정의
